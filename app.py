# app.py — Streamlit Math Quiz (로그인/랭킹 제거 버전, 시트+문제 기능만 유지)
...
def get_image_paths(raw:str)->list[str]:
    """
    시트의 'image' 셀에 파일명만 적으면 data/images/quiz 폴더에서 불러옴.
    여러개일 땐 ';'로 구분 가능.
    """
    if not raw: return []
...
# ===== 메인 화면 =====
st.title("🧮 수학 퀴즈")
st.caption("로그인 및 랭킹 없이 바로 문제를 풀 수 있는 버전입니다.")
...
# ===== 결과 화면 =====
    ...
    if not ss.logs:
        ...
        score = calc_weighted_score(df_log)
        st.write(f"총 {total}문항 | 정답 {correct} | 오답 {wrong} | 미기입 {blank} | 정답률 {rate:.1f}% | 점수 {score}")
