...
LEVELS = ["전체", "하", "중", "상"]
KEYWORDS = ["전체", "공통수학", "대수", "확률과 통계", "미적분"]
LEVEL_SCORE = {"하": 1, "중": 2, "상": 3}
...
if ss.stage == "home":
    ...
    keyword = st.radio("단원", KEYWORDS, index=KEYWORDS.index(ss.filters.get("keyword", "전체")))
...
# ===== 퀴즈 =====
    ...
    with b1:
        if st.button("제출 후 다음 문제"):
            commit(show_feedback=True, nextq=True)

    with b2:
        if st.button("제출 후 종료"):
            commit(show_feedback=True, nextq=False)
...
# ===== 정답 확인 =====
    ...
    if fb["correct"]:
        st.markdown(
            "<h1 style='color:limegreen; font-size:70px; text-align:center;'>✅ 정답!</h1>",
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            "<h1 style='color:red; font-size:70px; text-align:center;'>❌ 오답!</h1>",
            unsafe_allow_html=True
        )

        if fb["ua"] == "":
            st.markdown(
                "<h2 style='text-align:center;'>아무 답도 입력하지 않았어요.</h2>",
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f"<h3 style='text-align:center;'>정답은 <b style='color:orange;'>{fb['gt']}</b> 입니다.</h3>",
                unsafe_allow_html=True
            )
...
# ===== 결과 =====
    ...
    if not ss.logs:
        ...
        total = len(df_log)
        correct = (df_log["status"] == "correct").sum()
        blank = (df_log["status"] == "blank").sum()
